(ns jepsen.arangodb.tests.rw-register
...
(def dbName "rwRegister")
(def collectionName "rwCol")
(def attributeName "rwAttr")
...
(defrecord Client [db-created? collection-created? conn node]
  ...
  (invoke! [this test op]
    ; operation from {:type :invoke, :f :txn, :value [[:r 3 nil] [:w 3 6]}
    ; to {:type :ok, :f :txn, :value [[:r 3 1] [:w 3 6]]}
    (let [txn-vec (:value op)
      ...
      (try
        (let [ret-val (driver/submit-txn-rw conn dbName collectionName attributeName txn-vec txn-entity)]
          (if (not= ret-val nil)
...
(defn rw-register-test
  "Given an options map from the command line runner (e.g. :nodes, :ssh,
  :concurrency, ...), constructs a test map."
  [opts]
  (merge s/basic-test
         opts
         {:name            "arangodb-rw-register-test"
          :client          (Client. (atom false) (atom false) nil nil)
          :nemesis         (case (:nemesis-type opts)
                             :partition (nemesis/partition-random-halves)
                             :noop nemesis/noop)
          ;; from https://github.com/jepsen-io/jepsen/blob/main/jepsen/src/jepsen/tests/cycle/wr.clj
          :generator       (let [rw-gen (->> (wr/gen opts)
